...
# 3. fixture(미리 준비, 테스트 함수가 실행되기 전에 미리 준비) : 테이블 생성/삭제
# pytest가 자동으로 호출
...
def setup_db():
    ...
    yield 위 : 테스트 전 준비(테이블 생성) → reviewdb_test에 테이블 생성
               Base를 상속한 모든 모델(User, Item)의 테이블 생성
    yield 아래 : 테스트 후 정리(테이블 삭제)
                 다음에 pytest 실행 시 깨끗한 상태로 시작
    """
    ...
    # teardown : 테스트가 끝난 뒤 정리하는 작업
    print('\n[teardown] 테스트 테이블 삭제 완료')

# 4. fixture : TestClient 준비
@pytest.fixture(scope='module')
def client(setup_db):
    """get_db를 테스트용으로 교체하고 TestClient 생성"""
    ...
    # with 종료 후 dependency_override 초기화 : 다른 테스트 파일에 영향 주지 않도록 원상복귀
...
def test_create_user(client):
    ...
    # HTTP 상태코드 200(성공) 확인
    # assert : "이 값이 맞는지 확인해라" Python 내장 명령
    assert response.status_code == 200, f'예샹: 200, 실제: {response.status_code}'
    ...
    assert 'hashed_password' not in data    # 해시된 비밀번호 없음
...
def test_user_has_items(client):
    """유저 조회 시 items 배열에 아이템이 포함되어 있는지 확인(1:N 관계)"""
    response = client.get('/users/1')
