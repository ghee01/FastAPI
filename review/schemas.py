...
class ItemBase(BaseModel):
    title: str
    description: Optional[str] = None # 없어도 되는 필드, 쓴다면 문자열, 기본값 None
...
class ItemResponse(ItemBase):
    ...
    owner_id: int # 소유자 id를 포함

    # SQLAlchemy ORM 객체를 Pydantic 모델로 변환 허용
...
class UserCreate(UserBase):
    ...
    # password는 DB에 그대로 저장 X → crud.py에서 해싱 후 저장
    password: str
