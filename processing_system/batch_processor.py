# ===============================================================================
# processing_system/batch_processor.py
#   - "배치처리(Batch Processing)" 단계 예제 실습
...
def create_bus_area_summary() -> None:
    """
    버스 정류소 원본 데이터(bus_stop)를 "위치구분(구/군)" 기준으로 집계하여
    지역별 정류소 개수 및 좌표 누락 개수를 담은 테이블을 만든다

    SQL 설명:
    - COALESCE('위치구분', '미분류') : '위치구분' 값이 NULL이면 '미분류' 문자열로 대체
    - COUNT(*) FILTER (WHERE 조건) : PostgreSQL의 '조건부집계' 문법
                                        그룹 전체 개수와 별도로 그 그룹 안에서 조건을 만족하는 행만 따로 세고 싶을 때 사용
    """
    execute_sql(
        bus_engine,
        '''
...
# 이 파일을 python batch_processor.py처럼 직접 실행했을 때만 아래 코드가 동작한다
# (다른 파일에서 import batch_processor로 불러올 때는 실행되지 않는다 / 모듈 재사용을 위한 관용구)
