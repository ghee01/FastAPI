...
class Todo(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        # Todo.user_id가 User.id를 참조 (이 할일은 어떤 회원의 것인지 연결)
        ForeignKey('user.id'),
        nullable=True,  # 담당 회원을 지정하지 않고도 생성 가능
    )
...
class User(Base):
    ...
    todos: Mapped[list['Todo']] = relationship(
        back_populates='user',
        # cascade='all, delete-orphan' : 회원이 삭제되면 그 회원의 Todo들도 함께 삭제
