'''
todo_project/database/db_connection.py
...
# 세션 팩토리 - 호출할 때마다 새로운 DB 세션(작업 단위) 객체
SessionFactory = sessionmaker(
    autocommit=False,   # commit() 직접 호출 시 DB 반영
    autoflush=False,    # commit 전에 자동으로 SQL 미리 보내지 않는다
    expire_on_commit=False,  # commit 이후에도 객체 값을 메모리에서 계속 사용 가능
...
def get_session():
    ...
    # 함수 안에 yield가 있으면 yield 시점까지 실행한 뒤,
    # 그 값(session)을 라우터 함수의 매개변수로 전달 → 라우터 함수 처리가 끝나면
