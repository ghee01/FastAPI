...
- 시간이 지날수록 실제로 만든 Todo 데이터가 쌓이면서 모델이 점점 더 우리 말투 패턴에 맞게 똑똑해지는 과정을 볼 수 있다
'''
...
def load_original_data() -> pd.DataFrame:
    """최초 학습에 사용한 샘플 데이터. 재학습 때도 계속 기반 데이터로 포함시킨다"""
    df = pd.read_csv(DATA_PATH)
    return df[['title', 'category']]

def load_user_corrected_data() -> pd.DataFrame:
    """
    사용자가 final_category를 직접 지정한(=확인/수정한) Todo만 새 학습 데이터로 사용
...
def main():
    ...
    if len(corrected_df) == 0:
        print(f"[WARN] 재학습에 쓸 새 데이터가 없습니다. 재학습을 건너뜁니다.")
        return

    # 원본 데이터 + 사용자 수정 데이터
    combined_df = pd.concat([original_df, corrected_df], ignore_index=True)
    # 같은 제목의 Todo가 여러 번 등장할 경우, 가장 최근 값을 우선시한다
    ...
    metadata = {
        ...
        "n_user_corrected": len(corrected_df),
