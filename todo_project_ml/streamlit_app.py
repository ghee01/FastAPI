...
""", unsafe_allow_html=True)    # unsafe_allow_html=True : HTML/CSS가 먼저 적용

# ---- 세션 상태(session_state) 초기화 ----
# 스트림릿은 버튼을 누를 때마다 전체 스크립트가 위에서 아래로 다시 실행된다
# 로그인을 했다는 사실을 변수에 담아둔다
# st.session_state는 재실행되어도 값이 유지되는 유일한 저장 공간
# 여기에 토큰/로그인 여부 저장
...
def logout():
    """세션 상태를 비워서 로그아웃 처리 (서버에 요청 보낼 필요 X → JWT는 Stateless 방식)"""
    st.session_state.access_token = None
    st.session_state.user_email = None
...
def extract_error_message(res: requests.Response, fallback: str) -> str:
    ...
    - 일반적인 HTTPException: {'detail':[{'type':..., 'msg':'Value error, ...'}]} 형태
    - 깔끔하게 출력하기 위해 리스트를 순회하며 msg만 뽑고 'Value error, ' 접두어는 잘라낸다
    ...
    if isinstance(detail, list):
        messages = []
        for err in detail:
            # err이 dict라면(딕셔너리라면) msg 키의 값을 msg 변수에 넣어라,
            # 아니면 err을 문자열로 형변환 해서 msg 변수에 넣어라
            msg = err.get('msg', '') if isinstance(err, dict) else str(err)
            # 'Value error, '를 없애겠다
            msg = msg.replace('Value error, ', '')
            messages.append(msg)
        # messages가 있다면 사이사이에 엔터를 삽입, 아니면 fallback 변수에 있는 값을 반환 
        ...
        if submitted:
            try:
                ...
                if res.status_code == 200:  # 로그인 성공
                    ...
                    st.rerun()  # 화면을 즉시 새로고침해서 아래의 Todo 화면으로 전환
                else:
...
if total > 0:
    # st.progress : 진행률 표시 막대(progress bar)를 화면에 그려주는 위젯
...
for todo in todos:
    ...
    with check_col:
        # 체크박스 값이 바뀌는 순긴(=클릭하는 순간) PATCH 요청
    ...
    if predicted:
        ...
        with cat_col1:
            if final:
                # 사용자가 이미 확인/수정 완료한 경우
                st.caption(f'✅ 확정된 카테고리: **{final}**')
            else:
                # 아직 아무도 확인하지 않은, 모델의 예측 그대로인 상태
                st.caption(f'🤖 모델 예측: **{predicted}** (확인 필요)')
        with cat_col2:
            # 확정값이 있으면 그 값을, 없으면 예측값을 selectbox의 초기 선택값으로 사용
            ...
            selected = st.selectbox(
                ...
                label_visibility='collapsed'    # 라벨을 화면에 안 보이게(위 caption이 라벨 역할 대신)
            )

            # 사용자가 selectbox에서 값을 바꾼 경우만 서버에 확정 요청
            # → Todo.final_category를 채우는 지점
            # → 나중에 ml/retrain.py의 새 학습 데이터가 된다
