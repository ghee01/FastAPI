'''
todo_project/routers/todo.py

HTTP 요청을 받고 응답을 돌려주는 것만 담당
...
import shutil   # 파일/폴더를 복사, 이동, 삭제, 압축하는 고수준 파일 작업을 제공하는 표준 라이브러리
...
# 1. get_current_user_id() → 반환값 user_id에 대입
# 2. get_todo_service() → 반환값 service에 대입
# 3. get_todos_handler() 함수 본문 실행
