'''
todo_project/routers/ml.py

카테고리 예측 모델의 "정확도"를 확인할 수 있는 모니터링 전용 엔드포인트

MLOps 핵심 개념
- 모델을 배포하고 끝이 아니라, 배포 후에도 계속 지켜본다(모니터링)는 것이 이 파일의 존재 이유
- 실무에서는 정확도가 서서히 떨어지는 현상(model drift)을 감지하기 위해 이런 모니터링 지표를 반드시 별도로 관리한다
'''
...
# 지금은 실습용이라 누구나 접근 가능 → 실무라면 관리자 권한 체크 필요
...
def get_model_accuracy_handler(session=Depends(get_session)):
    """
    final_category가 채워진(=사용자가 확인/수정 완료한) Todo만 "정답이 있는" 데이터로 취급해서 정확도 계산 대상으로 삼는다
    predicted_category만 있고 final_category가 없는(=아직 아무도 확인 안 한) Todo는 정확도 계산에서 제외된다
    """
