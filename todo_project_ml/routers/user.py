'''
todo_project/routers/user.py

HTTP 요청/응답 담당
회원가입/로그인/토큰재발급/로그아웃 → UserService에서 진행
'''

# BackgroundTasks : 응답을 먼저 클라이언트에게 보내고, 그 이후에 처리해도 되는 작업을 뒤에서 실행하게 해준다
...
def send_welcome_email(email: str):
    ...
    실제 메일 발송 대신 5초 걸리는 작업으로 대체
    ← BackgroundTasks로 등록하면 클라이언트는 이 5초를 안 기다리고 바로 응답을 받는다
    """
    import time
    time.sleep(5)
...
def login_user_handler(
    ...
    return service.login(body)  # 이메일/비밀번호 검증부터 토큰 발급, refresh_token, DB저장까지 처리 후 리턴
