'''
todo_project/services/category_service.py
...
class CategoryPredictionService:
    def __init__(self, model):
        """
        main.py의 lifespan에서 joblib.load()로 딱 한 번 로드해준 sklearn Pipeline 객체를 그대로 주입받는다
        매 요청마다 이 클래스를 새로 만들어도 model 자체는 재사용되므로 무거운 로딩이 반복되지 않는다
        """
        self.model = model
