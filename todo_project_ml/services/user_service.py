'''
todo_project/services/user_service.py

회원가입/로그인/토큰재발급/로그아웃의 업무 규칙 담당 계층
'''
...
class UserService:
    ...
    def signup(self, body: UserSignUpRequest) -> User:
        ...
        if self.repository.find_by_email(body.email):
            # 이메일 중복 확인 → 존재하면 409(Conflict)로 거부
    ...
    def login(self, body:UserLoginRequest) -> dict:
        ...
        if not user or not verify_password(body.password, user.hashed_password):
            ...
            # 따로 알려주게 되면 공격자가 추측할 수 있기 때문에 보안상 구분 X
            raise HTTPException(
        ...
        # refresh_token을 DB에 저장 → 로그아웃이 가능하다
    ...
    def refresh(self, refresh_token: str) -> dict:
        ...
        첫번째 검증: decode_token(..., expected_type='refresh')
                    → 서명이 유효하고 만료되지 않은 'refresh'이 맞는지 확인 (access token을 넣으면 거부된다)
        두번째 검증: DB에 저장된 값과 일치하는지 확인
                    → 로그아웃을 했거나 이미 폐기된 토큰이면 서명 자체는 통과되어도 DB에 없으니 거부된다
        """
        ...
        return {'access_token':create_access_token(user_id)}    # 새 access token 발급

    def logout(self, refresh_token: str) -> None:
        """
        DB에 저장해둔 refresh_token을 지워서 "로그아웃" 구현
        (쿠키/세션 방식이 아니라 DB 기반이라는 것이 핵심
        JWT는 stateless, 서버가 스스로 토큰을 무효화한다는 개념이 원래는 없다
        보안을 위해 refresh_token만큼은 DB로 추적해서 로그아웃을 실제로 동작하게 만든다
        access_token 자체는 만료 전까지는 여전히 유효하다는 점이 한계)
        """
        user_id = decode_token(refresh_token, expected_type='refresh')
        user = self.repository.find_by_id(user_id)
        ...
        if user:
            ...
            self.repository.save(user)  # 로그아웃 한 것을 저장