'''
todo_project/services/todo_service.py

Todo 관련 "업무 규칙"을 담당하는 계층
DB 쿼리 자체는 직접하지 않고, TodoRepository에게 다 위임한다
없으면 "404 에러", 수정할 때 "title/is_done만 부분 반영" 등의 판단들 정의
...
class TodoService:
    ...
    def get_todo(self, todo_id: int, user_id: int) -> Todo:
        # 없으면 404 라는 업무 판단
        # Repository에서 None 반환 → 404 에러 (에러 낼지, 통과시킬지는 이 계층에서 처리)
    ...
    def create_todo(self, body: TodoCreateRequest, user_id: int) -> Todo:
        ...
        return self.repository.save(todo)   # DB에 저장 (서비스가 바로 저장 X, Repository한테 맡김)
