...
def get_player(db: Session, player_id: int):
    """player_id 하나로 선수 1명을 조회한다. 없으면 None 반환"""
    return db.query(models.Player).filter(
        models.Player.player_id == player_id
    ).first()   # player_id의 첫 번째 레코드(행)를 조회하고 반환

def get_players(db: Session, skip: int=0, limit: int=100,
                min_last_chaned_date: date=None, last_name: str=None, first_name: str=None):
    """
...
def get_performances(db: Session, skip: int=0, limit: int=100, min_last_changed_date: date=None):
    """경기성적에 대한 레코드 목록 조회 / get_players와 동일한 패턴"""
...
def get_leagues(db: Session, skip: int=0, limit: int=100,
    ...
    """
    리그 목록 조회, 각 리그에 속한 teams까지 한 번에 즉시 로딩(eager load)
    joinedload(models.League.teams) → League 조회할 때 SQL JOIN으로 Team까지 한 번의 쿼리로 같이 가져온다
    """
