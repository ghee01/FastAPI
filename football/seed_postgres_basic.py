...
#   - test_crud.py는 CSV 파일을 직접 읽지 않고,
#   - PostgreSQL DB에 이미 들어있는 데이터를 조회해서 테스트 한다
#   - 테스트 실행 전에 이 파일을 한 번 실행해서
#     CSV 데이터를 PostgreSQL 테이블에 넣어두어야 한다
# 
#   - 실행할 때마다 기존 테이블을 drop_all()로 삭제하고 다시 만든다
#   - DB에 중요한 데이터가 있다면 절대 실행하면 안된다
# =============================================================================

from __future__ import annotations  # 자료형 힌트
...
#   (어느 위치에서 실행하더라도 data 폴더를 안정적으로 찾기 위해 사용)
...
def parse_date(value: str) -> date:
    ...
    CSV에서 읽은 날짜 문자열을 Python date 객체로 변환한다
        CSV 파일 안의 날짜는 "2024-04-18" 같은 문자열
        models.py의 last_changed_date 컬럼은 SQLAlchemy Date 타입
        DB에 넣기 전에 문자열을 date 객체로 바꿔주는 것이 안전
    """
...
def read_rows(filename: str) -> list[dict[str, str]]:
    ...
    csv.DictReader()는 첫 줄의 컬럼명을 key로 사용하므로,
...
def seed() -> None:
    ...
    적재 순서가 중요
        ...
        - team.league_id는 league.league_id를 참조
        - performance.player_id는 player.player_id를 참조
        - team_player.team_id는 team.team_id를 참조
        - team_player.player_id는 player.player_id를 참조

        참조 당하는 부모 테이블을 먼저 넣고, 참조 하는 자식 테이블을 나중에 넣어야 한다
    """
    Base.metadata.drop_all(bind=engine)
    Base.metadata.create_all(bind=engine)

    # Session : SQLAlchemy ORM 객체를 DB에 저장하는 작업 공간
    # with 블록을 쓰면 작업이 끝난 뒤 세션이 자동으로 정리된다
