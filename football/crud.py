...
def get_player(db: Session, player_id: int):
    ...
    db.get(모델 클래스, 기본키 값) : 기본키로 딱 한 건만 조회할 때 사용하는 전용 메서드
    """
...
def get_players(db: Session, skip: int=0, limit: int=100,
    ...
    # .scalars() : 각 행에서 첫 번째 컬럼(여기서는 Player 객체 자체)만 꺼내서
    #              리스트처럼 다루게 해주는 결과 객체 반환
    # .all() : 그 결과를 파이썬 리스트로 변환해서 반환
    return db.scalars(stmt).all()

def get_performances(db: Session, skip: int=0, limit: int=100,
                     min_last_changed_date: date | None=None):
    """경기 성적(Performance) 레코드 목록을 조회"""
...
def get_league(db: Session, league_id: int | None=None):
    """
    리그 하나를 league_id로 조회
    league_id가 None인 경우, db.get(모델, None)을 그대로 호출하면
    의도치 않은 에러가 발생할 가능성이 높다
    미리 None을 걸러낸다
    """
...
def get_leagues(db: Session, skip: int=0, limit: int=100,
    ...
    """
    리그 목록 조회, 각 리그에 속한 팀까지 한 번에 즉시 로딩(eager load)
...
def get_teams(db: Session, skip: int=0, limit: int=100,
    ...
    """
    팀 목록 조회, league_id를 주면 특정 리그 소속 팀만 필터링

    --------------------------------------------------------
    team.team_players
        → "이 팀에 연결된 TeamPlayer(가입 기록) 목록"
        → 그 TeamPlayer 각각의 .player는 다시 "그 기록에 연결된 Player 객체"
