...
@pytest.fixture(scope='function')   # 함수 범위 테스트
def db_session():
    """
    테스트 함수마다 새 DB 세션 열고, 테스트 끝나면 닫는다
    scope='function' : 테스트 하나 당 세션 새로 만든다
                       세션 공유 X → 테스트 간 오염 걱정 적다
    """
...
def test_get_player(db_session):
    ...
    player = crud.get_player(db_session, 1001)
    assert player.player_id == 1001

def test_get_players(db_session):
    """2024-04-01 이후 변경된 선수 수가 seed 데이터 기준 1018명인지 확인"""
    players = crud.get_players(db_session, skip=0, limit=10000, min_last_chaned_date=test_date)
...
def test_get_players_by_name(db_session):
    ...
    first_name / last_name 두 조건을 동시에 걸어서 정확히 1명만 나오는지(동명이인 X)
...
def test_get_league(db_session):
    ...
    """
    league = crud.get_league(db_session, 5002)
