...
#   - team_player 처럼
#     "중간 연결 테이블에 추가 데이터가 있는 다대다 관계"를 다룰 때 사용
# -------------------------------------------------------------------
...
# mapped_column(..)
#   - 실제 DB 컬럼의 자료형/제약조건(기본키, 인덱스 등) 설정
# relationship(...)
...
class Player(Base):
    ...
    #   relationship() → DB 컬럼 X
    #   ex) Player.performances 라고 쓰면 이 선수의 Performance 객체 목록을
    #       파이썬의 리스트처럼 꺼내볼 수 있다는 의미
    # 
    ...
    # -----------------------------------------------------------------------
    # N:M 관계 (선수 여러명 ↔ 팀 여러 개) → Association Object 패턴
    # 
    # TeamPlayer 라는 클래스를 정식 ORM 클래스로 만들고,
    # Player ↔ TeamPlayer, Team ↔ TeamPlayer를 각각 1:N 관계로 연결한 뒤,
    # TeamPlayer를 "연결 기록이 저장된 중간 다리"로 활용
    # 
    # cascade='all, delete-orphan'
    #   - 선수가 team_player 에서 제거되거나 자체가 삭제되면,
    #     관련된 TeamPlayer(가입 기록) 행도 같이 정리한다
    #     (고아 상태로 남은 team_player 행이 안 생기게 한다)
    # -----------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------------------------------
    # association_proxy
    #   - team_players는 TeamPlayer(연결 기록) 객체의 리스트라서 "이 선수가 속한 팀 이름들만 보고 싶다"
    #     → player.team_players[0].team.team_name → 너무 길다
    # 
    # association_proxy('team_players', 'team')
    #   - 예전처럼 player.teams 라고만 써도 Team 객체 리스트를 바로 꺼내볼 수 있게 해주는 "편의 통로"
    # 
    # creator=lambda team: TeamPlayer(team=team)
    #   - player.teams.append(어떤 팀)처럼 "쓰기"로 사용할 때,
    #     내부적으로 어떤 TeamPlayer(team=팀, player=선수)를 자동으로 만들어서 연결해준다
    # ---------------------------------------------------------------------------------------------
...
class Performance(Base):
    ...
    fantasy_points: Mapped[float] = mapped_column(Float)    # 그 주차의 판타지 포인트 점수
    last_changed_date: Mapped[date] = mapped_column(Date)
...
class Team(Base):
    ...
    # creator : team.players.append(선수)로 쓸 때 TeamPlayer(player=선수)를 자동 생성
...
class TeamPlayer(Base):
    ...
    "이 연결이 언제 마지막으로 바뀌었는지"까지 저장하는 정식 ORM 클래스

    예전 버전 단순 secondary='team_player'로 하면 last_changed_date 고아 상태
