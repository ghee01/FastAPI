...
class Player(Base):
    ...
    # PostgreSQL에서는 String이 VARCHAR로 매핑
    # 길이를 지정하지 않은 VARCHAR(=길이 무제한)을 정식으로 지원하므로 별도 길이 지정 필요 X
...
class Performance(Base):
    ...
    player = relationship("Player", back_populates='performance')   # 1:N 관계
...
class TeamPlayer(Base):
    ...
    (team_id, player_id) 조합 전체를 복합 기본키로 사용
        - 같은 team_id, player_id 조합은 한 번만 존재 가능(중복 가입 방지)
    """
