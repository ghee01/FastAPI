...
# 
# 왜 이 도구들을 쓰는가?
# 1) Swagger UI에서 하나하나 클릭하고, 응답결과를 눈으로 읽고, 숫자를 세는 행위는
#    느리고 지치고 실수가 생길 수가 있다
#    이 확인 과정을 "코드로 자동화"
# 2) 회귀(regression) 방지
...
#       진짜 서버(uvicorn)를 키려면 포트를 열고, 네트워크 소켓을 열고, 실제로 통신을 해야 한다
#       → 느리고, 포트 충돌 같은 부수적인 문제도 생긴다
#       TestClient는 실제 네트워크 통신 없이 main.py의 app 객체를 파이썬 함수 호출 수준에서 그대로 실행
#       → 서버를 따로 켜둘 필요가 없다 → pytest 한 줄로 API 테스트가 가능하다
# =======================================================================================================
...
def test_read_players():
    ...
    limit=10000처럼 실제 선수 수(1018명)보다 훨씬 큰 값을 줘서
    페이지네이션 때문에 일부만 잘리지 않고 전체가 다 오는지 확인한다
    """
...
def test_read_players_by_name():
    ...
    # assert : 합격/불합격을 판정하는 채점 기준
    #          조건이 참이 아니면 여기서 즉시 실패로 멈춘다
    assert response.status_code == 200
