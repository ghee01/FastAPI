...
# 
# 5장
#   - API 문서화 (처음 보는 다른 개발자가 /docs(Swagger UI)만 보고도
#                  어떻게 사용하는지 이해할 수 있도록 OpenAPI 문서를 풍부하게 채운다)
#   - 문서화용 메타 데이터 추가
# 
...
#   → Swagger UI(/docs)와 Redoc(/redoc)의 상단 설명 영역
...
@app.get('/v0/players/', response_model=list[schemas.Player],
    ...
    description='''
        이 엔드포인트를 사용해 SWC 선수 목록을 조회합니다.
        매개변수를 이용해 목록을 필터링할 수 있습니다.
        이름은 고유하지 않습니다. skip과 limit을 사용해 API 페이징을 수행합니다.
...
@app.get('/v0/performances/', response_model=list[schemas.Performance],
    ...
    description='''
        이 엔드포인트를 사용해 SWC에서 선수의 주간 성적 목록을 조회합니다.
        skip과 limit을 사용해 페이징을 수행합니다.
...
def read_performances(db: Session=Depends(get_db),
                      skip: int=Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
                      limit: int=Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'),
                      minimum_last_changed_date: date=Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.')):
    """선수 성적 목록 조회"""
...
@app.get('/v0/leagues/', response_model=list[schemas.League],
    ...
    description='''
        이 엔드포인트를 사용해 SWC 판타지 풋볼 리그 목록을 조회합니다.
        skip과 limit을 사용해 페이징을 수행합니다.
        리그 이름은 고유하지 않습니다.
        리그 ID는 내부 ID이므로 개수 계산이나 로직에 사용하지 않습니다.
        순차적으로 보여주지 않습니다.
    ''',
...
def read_leagues(skip: int=Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
                 limit: int=Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'),
                 minimum_last_changed_date: date=Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.'),
                 league_name: str=Query(None, description='반환할 리그의 이름입니다. SWC에서 고유하지 않습니다.'),
                 db: Session=Depends(get_db)):
...
@app.get('/v0/teams/', response_model=list[schemas.Team],
    ...
    description='''
        이 엔드포인트를 사용해 SWC 판타지 풋볼 팀 목록을 조회합니다.
        skip과 limit을 사용해 페이징을 수행합니다.
        팀 이름은 고유하지 않습니다.
...
def read_teams(skip: int=Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
               limit: int=Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'),
               minimum_last_changed_date: date=Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.'),
               team_name: str=Query(None, description='반환할 팀의 이름입니다. SWC 전체에서 고유하지 않으나, 리그 내부에서는 고유합니다.'),
...
@app.get('/v0/counts/', response_model=schemas.Counts,
    ...
    description='''
        이 엔드포인트를 사용해 SWC 판타지 풋볼의 리그, 팀, 선수 수를 확인합니다.
        다른 조회 엔드포인트의 skip과 limit 매개변수와 함께 사용합니다.
        개수를 확인하려면 다른 API 대신 이 엔드포인트를 사용합니다.
    ''',
