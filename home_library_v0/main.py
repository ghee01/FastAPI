...
# ------------------------------------------------------------------------
# POST /books/scan → 사진을 업로드해서 책 한 권을 등록하는 API → Version 2
# ------------------------------------------------------------------------
...
def scan_book(image: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        #                  (디스크에 진짜 파일을 만들지 않고도 파일인 것처럼 열어볼 수 있다)
        # Image.open(...) → PIL(Pillow) 라이브러리로 이 바이트가 이미지로 열리는지 시도
        # with ... as plobe: → 확인이 끝나면 자동으로 메모리 정리 (파일 close)
    ...
    # OSError : 파일이 중간에 잘렸거나 손상된 경우 등 좀 더 넓은 범위의 파일 관련 에러
        ...
        # HTTP 상태코드 415(Unsupported Media Type, 지원하지 않는 파일 형식) 함수 실행 중단
        # raise를 만나는 순간, 저장과 DB 등록 실행 X
        raise HTTPException(415, '올바른 이미지 파일이 아닙니다.')

    # uploads 폴더 밑에 "원래 업로드된 파일명"으로 저장 경로를 만든다
    path = UPLOAD_DIR / image.filename

    # image.file.read() : 업로드된 파일의 실제 바이트 내용을 읽어온다
    ...
    db.add(book)    # 이 책 데이터를 저장 대기열에 올린다 (아직 DB에 실제로 시용되지는 않는다)
    db.commit()     # 대기열에 올린 내용을 실제로 DB에 확정 저장
    db.refresh(book)    # DB가 자동으로 채운 값(id, created_at)을 book 객체에 다시 불러와 채워준다
