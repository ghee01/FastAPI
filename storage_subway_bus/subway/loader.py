# ===============================================
# storage_subway_bus/subway/loader.py
# 
# 수집한 결과로 나온 파일 subway_long.csv를
# subway_raw 테이블에 적재
# ===============================================
...
##      - loader.py 내부에서만 쓰는 헬퍼 함수
##      - 다른 쪽 파일에서는 이 함수를 직접 호출할 일 없다
##      - private 함수 같은 느낌(강제 X)
##      - 내부에서 주로 사용하는 용도니, 외부에서는 직접 가져다 사용하지 마라
def _to_bool(value):
    """WEEKEND_MAP에 없는 값이 들어오면 에러를 내지 않고 기본값 False로"""
...
def _prepare_chunk(chunk: pd.DataFrame) -> list[dict]:
    ...
    ]].to_dict(orient='records')    # 행 단위로 하나씩 딕셔너리 만든다

# 함수 정의
def load_from_csv(path: str=INPUT_PATH, chunksize: int=CHUNKSIZE) -> dict:
    """
    ...
    for i, chunk in enumerate(pd.read_csv(path, encoding='utf-8-sig', chunksize=chunksize)):
        try:
            # 이번 배치(데이터를 한 번에 처리하지 않고 일정한 묶음 단위로 처리)를
            # DB 삽입용 딕셔너리 리스트로 가공
            ...
            with engine.begin() as conn:
                ...
                # UNIQUE 제약에 위반되는 행은 에러를 내지 않고 무시(skip)하도록 설정
                stmt = stmt.on_conflict_do_nothing(constraint='uq_subway_raw_key')

                # 실제 SQL 실행
                result = conn.execute(stmt)

            # rowcount : 실제로 삽입된 행의 개수(충돌로 스킵된 행은 포함 X)
            # 일부 환경에서는 rowcount가 None일 수 있어 방어적으로 처리(0 표시)
            ...
            # 예상치 못한 에러(ex: 자료형 불일치) 발생 시, 이번 배치만 실패로 기록 후 다음으로 넘어간다
...
# 이 파일을 직접 실행했을 때만 (python loader.py) 아래 코드가 동작
# 다른 모듈(파일)에서 import loader만 했을 때는 이 부분 실행 X
