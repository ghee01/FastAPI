# ===================================================================================
# storage_subway_bus/subway/verify.py
# 
# 지하철 적재 검증
#   - 필수 컬럼 NULL 여부
#   - 인원수 음수 여부
...
#   - 프로그램이 시작될 때(create_engine(...) 호출 시점) 연결 여러 개를 미리 만들어서
#     "풀(pool, 저장소)"에 담아둔다
#   - engine.connect()를 호출하면, 새로 연결을 만드는 게 아니라 풀에서 이미 만들어진
#     연결을 잠깐 빌려온다
#   - 다 쓰고 나면(with 블록이 끝나면) 연결을 풀에 다시 반납해서
...
def verify():
    # engine.connect()로 커넥션을 열고, with 블록을 벗어나면 자동으로 연결 반환(close)
    with engine.connect() as conn:

        # --- 1. 전체 적재 건수 확인 ---
        # .scalar() : 결과에서 첫 번째 행의 첫 번째 컬럼 값 하나만 추출
        #             (값이 하나만 나오는 쿼리에 사용)
        total = conn.execute(text('SELECT COUNT(*) FROM subway_raw')).scalar()

        # --- 2. 필수 컬럼별 NULL 개수 확인 ---
        # FILTER (WHERE 조건) : 하나의 SELECT 안에서 조건별로 COUNT를 따로 집계하는 PostgreSQL의 문법
        ...
        #   - 서브 쿼리에서 GROUP BY로 묶은 뒤, HAVING절을 사용해서
        #     "2건 이상 중복된 키 조합"만 걸러내고 바깥 쿼리에서 그 조합의 개수를 센다
        #   - "중복된 원본 행의 총 개수"가 아니라 "중복이 발생한 키 조합의 가짓수"
