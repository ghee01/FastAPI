...
2단계 - 중복 등록 응답을 에러가 아닌 메시지로 개선
        detail을 딕셔너리로 구조화해서, 기존 책의 제목/저자/출판사/상태를 각각 따로 꺼내 사용할 수 있게 한다
'''
import io
import uuid     # 충돌 없는 고유한 이름을 자동으로 만들어주는 라이브러리 (식별자)
...
def register_book(
    ...
    # 원본 파일명을 그대로 안 쓰고 이 문자열로 교체
    #   1) 같은 이름의 파일이 두 번 업로드 되어도 덮어쓰기 걱정 X, 데이터 손실 문제 해결
    #   2) 한글 파일명이 환경에 따라 깨지거나 다운로드 실패 나는 문제도 방지
