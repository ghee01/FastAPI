'''
todo_project/schema/request.py
...
class UserSignUpRequest(BaseModel):
    ...
    #   - description : 코드 실행에는 영향 X, docs/ 화면에만 표시되는 문서용 문자열
...
# ---- 로그인 요청 모델 ----
# 회원가입과 로그인을 분리한 이유 : 로그인에는 회원가입 때의 복잡한 비밀번호 규칙 검증 필요 X
# 로그인은 "이미 만들어진 비밀번호가 맞는지"만 확인
