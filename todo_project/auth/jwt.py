...
할 일 등록(POST), 할 일 완료 체크(PATCH), 할 일 확인(GET) 목적 → 로그인 → access_token 재발급
→ 목적 처리 → 처리 완료 → 로그아웃 access_token

시간 많이 경과
refresh_token = None (초기화)
로그인 → access_token + refresh_token 둘 다 발급
...
def _create_token(user_id: int, token_type: str, expires_delta: timedelta) -> str:
    ...
    이름 앞 _(언더스코어)의 역할
        - 이 파일 밖에서는 직접 부르지 말고, 아래 두 함수를 통해서만 호출하라는 파이썬의 관례
    """
    payload = {
        'user_id':user_id,
...
def decode_token(token: str, expected_type: str) -> int:
    """
    토큰을 검증하고 user_id 반환
    expected_type 반드시 넘겨받아서
    access 토큰인데 refresh 자리에 사용했다 같은 오남용을 여기서 걸러낸다 (핵심 보안 포인트)
    """
    try:
    ...
    return payload['user_id']   # 서명 검증 + 타입 검사 모두 통과 시 user_id만 반환
