'''
todo_project/repositories/todo_repository.py

todo 테이블에 대한 DB 쿼리만 담당하는 계층
쿼리 결과가 있으면 반환, 없으면 None(또는 빈 리스트) 반환까지만 책임지고
그 이후 반응에 대한 것은 Service에게 넘긴다

이렇게 나누는 이유
- Repository는 DB에서 데이터를 가져오는 방법만 알면 되고,
  HTTPException 같은 웹(HTTP) 관련 개념은 몰라도 된다
- 나중에 이 프로젝트를 웹이 아닌 다른 방식으로도 사용하면 Repository만 그대로 재사용
...
class TodoRepository:
    ...
    def save(self, todo: Todo) -> Todo:
        """
        새로 만든 Todo든, 기존 Todo를 수정하든 상관없이 저장
    ...
    def delete(self, todo: Todo) -> None:
        self.session.delete(todo)   # todo 삭제
        self.session.commit()